...
public class SmeltingRecipesProvider extends RecipeProvider implements IConditionBuilder {
    ...
    private void oreSmelting(List<ItemLike> ingredients, ItemLike result, float experience, String group, Consumer<FinishedRecipe> consumer) {
        for(ItemLike item : ingredients) SimpleCookingRecipeBuilder.smelting(Ingredient.of(item), result, experience, 200).group(group).unlockedBy(getHasName(item), has(item)).save(consumer, new ResourceLocation(DenseOres.MOD_ID, getItemName(result) + "_from_smelting_" + getItemName(item)));
    }
    ...
    private void oreBlasting(List<ItemLike> ingredients, ItemLike result, float experience, String group, Consumer<FinishedRecipe> consumer) {
        for(ItemLike item : ingredients) SimpleCookingRecipeBuilder.blasting(Ingredient.of(item), result, experience, 100).group(group).unlockedBy(getHasName(item), has(item)).save(consumer, new ResourceLocation(DenseOres.MOD_ID, getItemName(result) + "_from_blasting_" + getItemName(item)));
    }
